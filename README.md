## 版本调整：
* yaml文件：部分字段设置为非必传，data字段改为兼容字典和字符串，不会影响之前的用例 host默认取config的host, method默认POST,requestType默认json,is_run默认true,AssertType默认空，jsonpath不传默认取整个响应进行断言
* send_mail.py：修改为发送163邮箱，只要在config设置邮箱通知，发送人是163邮箱，就可以实现发送；修改了发送邮箱模板
* conftest.py：pytest默认是ascii码顺序运行test开头的用例，我在conftest新增了一段被注释的代码，如果需要指定顺序，就取消注释
* assertion改动：新增not_contains【不包含断言】，如果没有传status_code的断言。就会默认断言响应码==200
* 其他的修改查看git变动
