...
class Context:
    ...
    @classmethod
    def random_list(cls, key) -> int:
        """
        :return: 已生成的随机数
        """
        _data = random.randint(0, 5000)
        if key not in cls.dictRandom:
            cls.dictRandom[key] = random.randint(1, 5000)
        return cls.dictRandom[key]

    def getNewToken(cls) -> str:
        from utils import config
        """
        :return: 获取新token
        """
        phone = (str(cls.get_phone()))[:8]
        url = config.app_host + "/admin/register/user"
        json = {"password": "aaaaa888A", "phone": phone, "phoneCode": "886", "registerSource": 1,
                "registrant": 1, "validCode": "134578"}
        headers = {'content-type': 'application/json'}
        res = requests.request("POST", url, json=json, headers=headers, verify=False)
        # 请求登录接口
        url = config.app_host + "/auth/oauth2/token"
        data = {"username": "886:" + phone, "password": "czekOrsd+DrH",
                "grant_type": "password", "scope": "server", "phoneCode": "886"}
    ...
    @classmethod
    def backend_host(cls) -> str:
        from utils import config
        """获取后台的Host"""
        return config.backend_host

    @classmethod
    def google_auth(cls, secret) -> int:
        """获取app的host"""
