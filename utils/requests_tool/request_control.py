    ...
    def http_request(
        ...
        if is_run is True or is_run is None:
            ...
            try:
                ...
                text = (
                    f"<blockquote>\n<b>⭕接口访问异常   {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}</b>\n\n"
                    f"【用例描述】:\n{' ' * 8}{getattr(res,'detail', '') if res else self.__yaml_case.detail}\n"
                    f"【请求路径】:\n{' ' * 8}{getattr(res,'url', '') if res else self.__yaml_case.url}\n"
                    f"【请求方式】:\n{' ' * 8}{getattr(res,'method', '') if res else self.__yaml_case.method}\n"
                    f"【请求内容】:\n{' ' * 8}{str(getattr(res,'.request_body', '') if res else self.__yaml_case.data)}\n"
                )
                if not getattr(_res_data, 'status_code', None):
                    text += f"【异常原因】: \n{' ' * 8}{traceback.format_exc()}\n"
